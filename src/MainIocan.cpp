...
int contlora=0;
int vannesMode = 0; // 0 = TOGGLE (default), 1 = FORCE_ON, 2 = FORCE_OFF
...
String getVanneModeName(void* state) ;
...
WebVar varWeb[] = {
   ...
   {"offsend", &off, "int", "RW",1.0,NULL,"cnt"}  ,
    {"VannesMode", &vannesMode, "int", "RW",1,getVanneModeName,"mode"}  ,
        {"VANNE1", nullptr, "int", "RW",1,NULL,"state"},
        {"VANNE2", nullptr, "int", "RW",1,NULL,"state"},
        {"VANNE3", nullptr, "int", "RW",1,NULL,"state"},
        {"VANNE4", nullptr, "int", "RW",1,NULL,"state"},
...
String generateHTMLTable() {
    ...
    // Boutons pour controle des vannes: TOGGLE / FORCE ON / FORCE OFF
    html += "<div style='margin-top:8px;'>";
    html += "<button onclick=\"setVannesMode(0)\">Vannes: Toggle</button> ";
    html += "<button onclick=\"setVannesMode(1)\">Vannes: Force ON</button> ";
    html += "<button onclick=\"setVannesMode(2)\">Vannes: Force OFF</button>";
    html += "</div>";

    // Boutons individuels pour chaque vanne
    html += "<div style='margin-top:8px;'>";
    for (int i = 1; i <= 4; i++) {
        html += "V" + String(i) + ": ";
        html += "<button onclick=\"setVanne(" + String(i) + ",1)\">ON</button> ";
        html += "<button onclick=\"setVanne(" + String(i) + ",0)\">OFF</button> ";
        html += "<button onclick=\"setVanne(" + String(i) + ",-1)\">TOGGLE</button> ";
    }
    html += "</div>";
 ...
 //   html += "<div>command:" + command+ "</div>" ;
  //   html += "<div>json:" + generateJSONString()+ "</div>" ;
    ...
    // Fonction pour changer le mode des vannes via boutons
    html += "function setVannesMode(mode) { fetch('/update?name=VannesMode&value=' + mode).then(response => { if (!response.ok) console.error('Failed to set vannes mode'); }).catch(e=>console.error(e)); };";
    // Fonction pour controler une vanne individuelle (value: 1=ON,0=OFF,-1=TOGGLE)
    html += "function setVanne(index, value) { var name='VANNE'+index; if (value== -1) { fetch('/value?name='+name).then(r=>r.text()).then(t=>{ var cur = parseInt(t); var nv = cur?0:1; fetch('/update?name='+name+'&value='+nv).then(()=>document.getElementById('value_'+name).textContent = nv); }); } else { fetch('/update?name='+name+'&value=' + value).then(()=>document.getElementById('value_'+name).textContent = value); } };";
    html += "setInterval(refreshValues, 2000); function refreshValues() {";
    
    // Instructions JavaScript pour mettre à jour les valeurs
    for (int i = 0; i < sizeof(varWeb) / sizeof(varWeb[0]); i++) {
        String vname = String(varWeb[i].name);
        // If variable is a VANNE, show ON/OFF instead of 1/0
        if (vname.startsWith("VANNE")) {
            html += "fetch('/value?name=" + vname + "').then(response => response.text()).then(data => { document.getElementById('value_" + vname + "').textContent = (data.trim()=='1'?'ON':'OFF'); });";
        } else if (vname == "VannesMode") {
            html += "fetch('/value?name=VannesMode').then(response => response.text()).then(data => { document.getElementById('value_VannesMode').textContent = data; });";
        } else {
            html += "fetch('/value?name=" + vname + "').then(response => response.text()).then(data => document.getElementById('value_" + vname + "').textContent = data);";
        }
    }
...
// Vannes configuration
const int VANNE_COUNT = 4;
const int vannePins[VANNE_COUNT] = {VANNE1, VANNE2, VANNE3, VANNE4};
int vanneState[VANNE_COUNT] = {LOW, LOW, LOW, LOW};
...
void setup() {
    ...
    for (int i = 0; i < VANNE_COUNT; i++) {
        pinMode(vannePins[i], OUTPUT);
        digitalWrite(vannePins[i], LOW);
    }

    // Lier les entrées web `VANNE1..4` aux variables `vanneState`
    for (int i = 0; i < varCount; i++) {
        if (strcmp(varWeb[i].name, "VANNE1") == 0) varWeb[i].var = &vanneState[0];
        if (strcmp(varWeb[i].name, "VANNE2") == 0) varWeb[i].var = &vanneState[1];
        if (strcmp(varWeb[i].name, "VANNE3") == 0) varWeb[i].var = &vanneState[2];
        if (strcmp(varWeb[i].name, "VANNE4") == 0) varWeb[i].var = &vanneState[3];
    }
   ...
   // Configuration du serveur web
    ...
    server.on("/update", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (request->hasParam("name") && request->hasParam("value")) {
            ...
            // Si on a changé une VANNE, appliquer immédiatement sur la broche
            if (name.startsWith("VANNE")) {
                int idx = name.substring(5).toInt(); // "VANNE1" -> 1
                if (idx >= 1 && idx <= VANNE_COUNT) {
                    // Trouver la variable correspondante pour lire sa valeur
                    for (int j = 0; j < sizeof(varWeb) / sizeof(varWeb[0]); j++) {
                        if (strcmp(varWeb[j].name, name.c_str()) == 0 && strcmp(varWeb[j].type, "int") == 0) {
                            int v = *((int*)varWeb[j].var);
                            digitalWrite(vannePins[idx - 1], v ? HIGH : LOW);
                            break;
                        }
                    }
                }
...
// Variables pour le toggle des vannes
unsigned long previousVanneMillis = 0;
// (VANNE_COUNT, vannePins and vanneState declared earlier)
...
// Traducteur pour l'état des vannes
String getVanneModeName(void* state) {
    int m = *(int*)state;
    switch (m) {
        case 0:
...
void loop() {
    ...
    // Gestion des vannes `VANNE1..4` environ toutes les 1 seconde
    if (currentMillis - previousVanneMillis >= 1000) {
        previousVanneMillis = currentMillis;
        switch (vannesMode) {
            case 0: // TOGGLE
                for (int i = 0; i < VANNE_COUNT; i++) {
                    vanneState[i] = !vanneState[i];
                    digitalWrite(vannePins[i], vanneState[i] ? HIGH : LOW);
                }
                break;
            case 1: // FORCE_ON
                for (int i = 0; i < VANNE_COUNT; i++) {
                    vanneState[i] = HIGH;
                    digitalWrite(vannePins[i], HIGH);
                }
                break;
            case 2: // FORCE_OFF
                for (int i = 0; i < VANNE_COUNT; i++) {
