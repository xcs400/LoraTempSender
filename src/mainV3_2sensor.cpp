...
const int OLEDRST = RST_OLED;
const int POMPE = 48;               // not used
...
sensors.begin();
  String NodeId = "LoraDoubleTemp" ;  
  sensors.requestTemperatures(); 
  float temperatureC = sensors.getTempCByIndex(0);
  float temperature2 = sensors.getTempCByIndex(1);

 //  msg = "{\"model\":\"ESP32TEMP\",\"id\":\"" + NodeId + "\",\"T1\":" + String(temperatureC) +  "\",\"T2\":" + String(temperature2) +   "\",\"Elapsed\":" + String((int)round((double)bootCount*USDELAY/1000000)) + ",\"Vbatt\":" + String(voltage) + ",\"Charge%\":" + String(bars) + "}";
//  msg = "{\"model\":\"ESP32TEMP\",\"id\":\"" + NodeId + "\",\"T1\":" + String(temperatureC) +  "\",\"T2\":" + String(temperature2) +   "\",\"Elapsed\":" + String((int)round((double)bootCount*USDELAY/1000000)) + ",\"Vbatt\":" + String(voltage) + "}";
 ...
 //   Serial.println(String(msg));
    ...
    display.drawString(0, 15, "t1: " + String(temperatureC) + " C");
    display.drawString(64, 15, "t2: " + String(temperature2) + " C");
    display.display();
...
// adc_power_off();
  ...
  // Transmission LoRa (bloquante pour s'assurer que c'est fini avant le sleep)
  int transmissionState = radio.transmit(msg);
  Serial.println("Transmitting: " + msg);
  Serial.printf("Transmission state= %u\n", transmissionState);
...
void loop(void) {
  // Vide pour économiser l'énergie (deep sleep dans setup)
}
